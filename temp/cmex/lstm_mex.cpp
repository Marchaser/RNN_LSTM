...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	GET_INT(MEX_COMPUTE_MEMORY_SIZE);
	if (MEX_TASK == MEX_COMPUTE_MEMORY_SIZE)
	{
		COMPUTE_MEMORY_SIZE();
		return;
	}
...
void COMPUTE_MEMORY_SIZE()
{
	...
	// Construct network
	LstmLayer<float> lstmLayer1(xDim, hDim, T, batchSize);
	SoftmaxLayer<float> softmaxLayer(hDim, yDim, T, batchSize);

	int memorySize = lstmLayer1.computeMemorySize() + softmaxLayer.computeMemorySize();

	PUT_SCALAR(memorySize);
...
void PREDICT()
{
	...
	lstmLayer1.weights_x = ptr;
	ptr += hDim * 4 * xDim;

	lstmLayer1.weights_h = ptr;
	ptr += hDim * 4 * hDim;

	lstmLayer1.biases = ptr;
	ptr += hDim * 4;

	softmaxLayer.weights = ptr;
	ptr += yDim*hDim;

	softmaxLayer.biases = ptr;
	ptr += yDim;

	// Copy data
	memcpy(lstmLayer1.x_t, _xData, sizeof(float)*xDim*batchSize*T);
	memcpy(softmaxLayer.y_t, _yData, sizeof(float)*yDim*batchSize*T);
	...
	// Network linkage
	memcpy(softmaxLayer.h_t, lstmLayer1.h_t, sizeof(float)*hDim*batchSize*T);
	for (int t = 1; t <= T; t++)
		status = softmaxLayer.forward_pass(t);

	// Copy to output
	memcpy(_yhat_t, softmaxLayer.yhat_t, sizeof(float)*yDim*batchSize*T);
...
#pragma omp parallel for
	...
	for (int thread_id = 0; thread_id < NumThreads ; thread_id++)
	{
		if (lstmLayer1_thread[thread_id] == 0)
			lstmLayer1_thread[thread_id] = new LstmLayer<float>(xDim, hDim, T, batchSize);
		if (softmaxLayer_thread[thread_id] == 0)
			softmaxLayer_thread[thread_id] = new SoftmaxLayer<float>(hDim, yDim, T, batchSize);
		...
		lstmLayer1_thread[thread_id]->weights_x = ptr;
		ptr += hDim * 4 * xDim;

		lstmLayer1_thread[thread_id]->weights_h = ptr;
		ptr += hDim * 4 * hDim;

		lstmLayer1_thread[thread_id]->biases = ptr;
		ptr += hDim * 4;

		softmaxLayer_thread[thread_id]->weights = ptr;
		ptr += yDim*hDim;

		softmaxLayer_thread[thread_id]->biases = ptr;
		ptr += yDim;

		// dweights
		ptr = _dweights_thread + weights.numElements()*thread_id;
		lstmLayer1_thread[thread_id]->dweights_x = ptr;
		ptr += hDim * 4 * xDim;

		lstmLayer1_thread[thread_id]->dweights_h = ptr;
		ptr += hDim * 4 * hDim;

		lstmLayer1_thread[thread_id]->dbiases = ptr;
		ptr += hDim * 4;

		softmaxLayer_thread[thread_id]->dweights = ptr;
		ptr += yDim*hDim;

		softmaxLayer_thread[thread_id]->dbiases = ptr;
		ptr += yDim;
	}
...
#pragma omp parallel for
	...
	for (int thread_id = 0; thread_id < NumThreads; thread_id++)
	{
		...
		*/
		...
		// Network linkage
		memcpy(lstmLayer1->dtop_t, softmaxLayer->dh_t, sizeof(float)*hDim*batchSize*T);
		for (int t = T; t >= 1; t--)
